...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    case 'ADD_TO_CART':
      atIndex = newState.products.findIndex(
        (product) => product.name === action.payload.name,
      );
      if (atIndex > -1) {
        let product = newState.products[atIndex];
        product.inStock = product.inStock - 1;
        newState.products = [
          ...newState.products.slice(0, atIndex),
          product,
          ...newState.products.slice(atIndex + 1),
        ];
      }
      break;
    case 'REMOVE_FROM_CART':
      ...
      if (atIndex > -1) {
        let product = newState.products[atIndex];
        product.inStock = product.inStock + action.payload.quantity;
        newState.products = [
          ...newState.products.slice(0, atIndex),
          product,
          ...newState.products.slice(atIndex + 1),
